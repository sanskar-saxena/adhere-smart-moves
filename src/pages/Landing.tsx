...
import { fadeUpCustom, viewportOnce, SPRING_EASE } from "@/lib/motion";
...
const pricingPlans = [
  {
    name: "Free", price: "₹0", period: "forever", highlighted: false, cta: "Start Free",
    features: ["3 menu scans / week", "Daily adherence score", "Basic recovery plans", "Weight & waist tracking"],
  },
  {
    name: "Pro", price: "₹299", period: "/month", highlighted: true, cta: "Try Pro Free for 7 Days",
  ...
  {
    name: "Coach", price: "₹999", period: "/month", highlighted: false, cta: "Contact Sales",
...
const LandingNav = ({ onGetStarted }: { onGetStarted: () => void }) => (
  <header className="fixed top-0 left-0 right-0 z-50 border-b border-border/50 bg-gradient-glass">
    <div className="container flex items-center justify-between h-16">
      <AdhereLogo size="sm" />
      <div className="hidden md:flex items-center gap-8">
        {["features", "pricing", "how"].map((id) => (
          <a key={id} href={`#${id}`} className="text-[13px] font-medium text-muted-foreground hover:text-foreground transition-colors duration-200 capitalize">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="premium" size="sm" onClick={onGetStarted}>Get Started</Button>
      </div>
    </div>
  </header>
);

const LandingHero = ({ onGetStarted }: { onGetStarted: () => void }) => (
  <section className="pt-36 pb-24 md:pt-48 md:pb-36 bg-gradient-hero relative overflow-hidden">
    ...
    <div className="container relative">
      <motion.div className="max-w-3xl mx-auto text-center" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.12 } } }}>
        <motion.div variants={fadeUpCustom} custom={0}>
        ...
        <motion.h1 variants={fadeUpCustom} custom={1} className="text-display-sm md:text-display lg:text-display-lg font-extrabold text-foreground !leading-[1.08]">
          You know what to eat.
          <br />
          <span className="text-gradient-primary">You just can't do it in real life.</span>
        </motion.h1>
        <motion.p variants={fadeUpCustom} custom={2} className="mt-7 text-base md:text-lg text-muted-foreground max-w-xl mx-auto leading-relaxed">
        ...
        <motion.div variants={fadeUpCustom} custom={3} className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
        ...
        <motion.div variants={fadeUpCustom} custom={4} className="mt-16 flex items-center justify-center gap-10 md:gap-14">
          {socialProof.map((s) => (
...
const LandingHowItWorks = () => (
  <section id="how" className="py-24 md:py-32">
    <div className="container">
      <motion.div className="text-center mb-18" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ duration: 0.5, ease: SPRING_EASE }}>
      ...
      <div className="grid md:grid-cols-3 gap-10 max-w-4xl mx-auto">
        {howSteps.map((item, i) => (
          <motion.div key={item.step} className="text-center md:text-left" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.15, duration: 0.5, ease: SPRING_EASE }}>
  ...
  <section id="features" className="py-24 md:py-32 bg-muted/40">
    <div className="container">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto">
        {features.map((f, i) => (
          <motion.div key={f.title} className="group rounded-2xl border bg-card p-6 shadow-card transition-all duration-300 hover:shadow-card-hover hover:-translate-y-0.5" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.06, duration: 0.5, ease: SPRING_EASE }}>
...
const LandingPricing = ({ onGetStarted }: { onGetStarted: () => void }) => (
    ...
    <div className="container">
      ...
      <div className="grid md:grid-cols-3 gap-5 max-w-4xl mx-auto items-start">
        {pricingPlans.map((plan, i) => (
          <motion.div key={plan.name} className={`rounded-3xl border p-7 flex flex-col ${plan.highlighted ? "border-primary/20 bg-gradient-card shadow-xl shadow-primary/5 relative ring-1 ring-primary/10" : "bg-card shadow-card"}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.1, duration: 0.5, ease: SPRING_EASE }}>
            {plan.highlighted && (
            ...
            <Button className="mt-7 w-full" variant={plan.highlighted ? "premium" : "outline"} size="lg" onClick={onGetStarted}>
              {plan.cta}
            </Button>
          </motion.div>
        ))}
...
const LandingCTA = ({ onGetStarted }: { onGetStarted: () => void }) => (
  <section className="py-24 md:py-32 bg-muted/40 relative overflow-hidden">
    ...
    <div className="container relative">
      <motion.div className="max-w-2xl mx-auto text-center" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ duration: 0.6, ease: SPRING_EASE }}>
        ...
        <Button variant="premium" size="xl" onClick={onGetStarted} className="mt-10">
